 ...
 * 
 */
@EqualsAndHashCode(callSuper = true)//@EqualsAndHashCode是用于生成equals和hashCode方法的注解。@EqualsAndHashCode注解可以自动生成equals和hashCode方法，以实现对象的相等性
@ApiModel(value = "SocialUser", description = "用户第三方用户信息对象 iot_social_user")
@Accessors(chain = true)//@Accessors注解用于配置getter和setter方法的生成方式。@Accessors(chain = true)表示setter方法可以使用链式调用
@Data
