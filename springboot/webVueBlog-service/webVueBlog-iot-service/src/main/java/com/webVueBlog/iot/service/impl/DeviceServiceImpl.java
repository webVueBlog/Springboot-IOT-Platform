...
import com.webVueBlog.common.constant.ProductAuthConstant;
import com.webVueBlog.common.core.domain.AjaxResult;
import com.webVueBlog.common.core.domain.entity.SysRole;
...
import com.webVueBlog.common.enums.DataEnum;
...
import com.webVueBlog.common.utils.CaculateUtils;
...
import com.webVueBlog.common.utils.json.JsonUtils;
...
import com.webVueBlog.iot.mapper.DeviceUserMapper;
import com.webVueBlog.iot.mapper.EventLogMapper;
import com.webVueBlog.iot.model.AuthenticateInputModel;
import com.webVueBlog.iot.model.DeviceAllShortOutput;
import com.webVueBlog.iot.model.DeviceMqttConnectVO;
import com.webVueBlog.iot.model.DeviceMqttVO;
import com.webVueBlog.iot.model.DeviceRelateUserInput;
import com.webVueBlog.iot.model.DeviceShortOutput;
import com.webVueBlog.iot.model.DeviceStatistic;
import com.webVueBlog.iot.model.ProductAuthenticateModel;
import com.webVueBlog.iot.model.ThingsModelItem.Datatype;
import com.webVueBlog.iot.model.ThingsModelItem.EnumItem;
import com.webVueBlog.iot.model.ThingsModelItem.ThingsModel;
import com.webVueBlog.iot.model.ThingsModels.PropertyDto;
import com.webVueBlog.iot.model.ThingsModels.ThingsModelShadow;
import com.webVueBlog.iot.model.ThingsModels.ThingsModelValueItem;
import com.webVueBlog.iot.model.ThingsModels.ValueItem;
import com.webVueBlog.iot.model.UserIdDeviceIdModel;
import com.webVueBlog.iot.service.*;
...
public class DeviceServiceImpl implements IDeviceService {
            ...
            if (null != dto.getIsHistory()) {
                ...
                switch (modelType) {
                    ...
                    case EVENT:
                        ...
                        event.setSerialNumber(input.getDeviceNumber().contains("_") ? device.getGwDevCode() : input.getDeviceNumber());
    ...
    public Device insertDevice(Device device) {
        ...
        /*如果设备属于网关设备并且关联有采集点模板将根据采集点模板自动添加子设备*/
        if (result > 0) {
            DeviceTemplate deviceTemplate = deviceTemplateMapper.selectDeviceTemplateByProduct(device.getProductId());
            if (null != deviceTemplate) {
                VarTempSalve tempSalve = new VarTempSalve();
                tempSalve.setDeviceTempId(deviceTemplate.getTemplateId());
                /**从机列表*/
                List<VarTempSalve> salveList = varTempSalveService.selectVarTempSalveList(tempSalve);
                if (!CollectionUtils.isEmpty(salveList)) {
                    salveList.forEach(var -> {
                        //从机设备编号唯一 设备编号+_+从机编号
                        Device subDev = new Device();
                        BeanUtils.copyProperties(device, subDev);
                        subDev.setSerialNumber(device.getSerialNumber() + "_" + var.getSlaveAddr());
                        subDev.setGwDevCode(device.getSerialNumber());
                        subDev.setDeviceName(var.getSlaveName());
                        subDev.setSlaveId(var.getSlaveAddr());
                        deviceMapper.insertDevice(subDev);
                    });
                }
            }
        }
    ...
    public List<ThingsModelValueItem> cacheDeviceStatus(Long productId, String serialNumber) {
        ...
        //如果 properties functions不为空进行转换，否则报空指针
        if (!CollectionUtils.isEmpty(properties)) {
