...
public class MqttPublish implements MqttHandler {
    ...
    public void handler(ChannelHandlerContext ctx, MqttMessage message) {
        ...
        if (topicName.endsWith(DaConstant.MQTT.PROPERTY_GET_SIMULATE)) {
            ...
            /*累计接收消息数*/
            redisCache.incr2(DaConstant.REDIS.MESSAGE_RECEIVE_TOTAL, -1L);
            redisCache.incr2(DaConstant.REDIS.MESSAGE_RECEIVE_TODAY, 60 * 60 * 24);
        }
    }
    ...
    public void sendToMQ(MqttPublishMessage message) {
        ...
        /*只处理上报数据*/
        if (!topicName.endsWith(DaConstant.MQTT.UP_TOPIC_SUFFIX)) {
            return;
        }
        ...
        if (topicName.endsWith(DaConstant.TOPIC.MSG_REPLY) ||
                ...
                topicName.endsWith(DaConstant.TOPIC.UPGRADE_REPLY)) {
            /*设备应答服务器回调数据*/
    ...
    private void pubRetain(MqttPublishMessage message) {
        redisCache.incr2(DaConstant.REDIS.MESSAGE_RETAIN_TOTAL, -1L);
